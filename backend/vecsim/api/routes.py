...
from vecsim import config
from vecsim import TEXT_MODEL
from vecsim.schema import SimilarityRequest, SearchRequest, UserTextSimilarityRequest
from vecsim.models import Product
from vecsim.query import create_query, count
...
async def products_from_results(total, results) -> list:
    ...
    return {
        "total": total,
        "products": [await process_product(p, i) for i, p in enumerate(results.docs)],
    }
...
async def get_products(
    limit: int = 20, skip: int = 0, gender: str = "", category: str = ""
):
    products = []
    expressions = []
    ...
    # Run query
    products = (
        await Product.find(*expressions)
...
async def text_search_products(search: SearchRequest):
    ...
    products = (
        await Product.find(Product.product_metadata.name % search.text)
        .copy(offset=0, limit=num_products)
        .execute()
    )
    return products
...
async def find_products_by_image(similarity_request: SimilarityRequest) -> t.Dict:
    ...
    count_query = count(
        gender=similarity_request.gender, category=similarity_request.category
    )
...
async def find_products_by_user_text(
